...
- **子が現れたら右に開く**: 親をメイン (またはペイン) で開いている間に、その issue の子が
  サイドバーに新しく現れる (spawn された / 題が付いて規約に合う形になった) と、自動でペインに開く。
  起動時点で既にある子は対象外。現れた子のペインが閉じていれば開く (題が変わって再び現れたときも同じ)
- **⊞ (親の行)**: 親と子をまとめて手で右に開く
- **モデルの強制**: セッション画面 (メインもペインも) を開いたとき、composer の「モデル: …」が
  上の期待と違えば、モデルメニューから切り替える (`[S]` なのに Opus → Sonnet、親なのに Sonnet → Opus)。
  規約に合わないタイトルは触らない。判定は 1 画面につき 1 回 (その後ユーザーが手で変えたものは尊重)
