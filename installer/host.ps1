<#
  claude-code-web-split-view の native messaging host (ippoan/gh-actions-live の host.ps1 の移植)。
  拡張 (chrome.runtime.sendNativeMessage) から呼ばれ、ローカルで update.ps1 を走らせる。
  拡張はディスクに書けないので、ツールバーのボタン更新の実体はこれ。
  ...
  MSI が HKCU\Software\Google\Chrome\NativeMessagingHosts\jp.ippoan.claude_code_web_split_view に
